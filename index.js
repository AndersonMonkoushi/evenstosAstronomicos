...
const chuvasVisiveisHoje = colecaoChuvasDeMeteoros.filter((chuva) =>
    verificaChuvaVisivelPorData(chuva, dataAtual)
);

const chuvasVisiveisProximos2Meses = colecaoChuvasDeMeteoros.filter((chuva) =>
...
if (chuvasVisiveisHoje.length > 0) {
    let msg = "\nEncontramos ";
    msg +=
        chuvasVisiveisHoje.length == 1 ?
        "1 chuva de meteoros que pode ser vista hoje" :
        chuvasVisiveisHoje.length +
        " chuvas de meteoros que podem ser vistas hoje";
